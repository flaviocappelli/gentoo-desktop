...
# Version 1.2
#
# Recursively check and modify desktop entries.
    ...
    for item in *; do
        if [ -d "$1/$item" ]; then
            ...
            if [ "$ext" == ".sh" ]; then
                ...
                rettxt=$(patch -d/ -p0 --follow-symlinks --batch --forward --dry-run < "$1/$item") && ok=1
                if [ $ok -eq 1 ]; then
                    patch -d/ -p0 --follow-symlinks --batch --forward < "$1/$item" >/dev/null
...
echo -e "\nSimplify .desktop files..."
/root/SYSTEM/MENU/simplify_desktopfiles.py

echo "Start patching menu entries..."
...
rm -f /usr/share/applications/*.orig
